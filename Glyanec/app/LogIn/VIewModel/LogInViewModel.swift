...
class LogInViewModel: ViewModel, BaseAlert, CLLocationManagerDelegate {
    ...
    func phoneError() {
        let message = "Invalidate phone number."
        self.displayErrorNotification(withText: message, sticky: false, action: nil, actionName: "profile_ok".localize)
    }
    
    func signup(phone: String) {
//        let requestModel = RequestSignUpModel(phone: phone, mobile: "mobile")
//        signUpRequest(requestModel: requestModel)
    }
    
//    func signUpRequest(requestModel: RequestSignUpModel) {
//        LoadingSpinner.shared.startActivity()
//        NetworkAuth.getCode(parameters: requestModel)
//            .done { (success) in
//                LoadingSpinner.shared.stopActivity()
////                Coordinator.shared.goToResetPasswordViewController(phone:requestModel.phone)
//
//        }.catch { (error) in
//            LoadingSpinner.shared.stopActivity()
...
//    }
     ...
     private func login(model: RequestAuthLoginModel) {
            LoadingSpinner.shared.startActivity()
            NetworkAuth.signIn(parameters: model)
                .done { (responseModel) in
                    LoadingSpinner.shared.stopActivity()
    //                UserDefaults.standard.set(true, forKey: KeyConstant.isLogin)
    //                UserDefaults.standard.synchronize()
    //                let needSaveLogin = UserDefaults.standard.object(forKey: KeyConstant.rememberAccount)
    //                if needSaveLogin == nil {
    //                    UserDefaults.standard.set(true, forKey: KeyConstant.rememberAccount)
    //                    self.saveLogin()
    //                } else if let needSave = needSaveLogin as? Bool, needSave {
//                        self.saveLogin()
    //                }
    //                CurrentUser.shared.updateCurrentUserModel()
    ...
    func loginRequest(model: RequestAuthLoginModel) {
        let requestModel = RequestAuthLoginModel(phone: model.phone,
                                                 password: model.password)
        login(model: requestModel)
    }
    
    private func saveLogin(model: RequestAuthLoginModel) {
        KeyChain.set(key: KeyConstant.login, string: model.phone)
